...
export default {
  ...
  methods: {
    ...
    getSummary() {
      let days = this.$moment(this.endDate).diff(
        this.$moment(this.startDate),
        'day'
      ) //相差天数
      for (let i = 0; i <= days; i++) {
        this.xAxisData.push(
          this.$moment(this.startDate).add(i, 'days').format('YYYY-MM-DD')
        )
            ...
            response.data.forEach((item2, index2, arr2) => {
              if (item2.refDate.indexOf(item) >= 0) {
                this.$set(
                  this.seriesData5,
                  index,
                  this.seriesData5[index] + item2.newUser
                )
                this.$set(
                  this.seriesData6,
                  index,
                  this.seriesData6[index] + item2.cancelUser
                )
              }
            })
          })
          // 基于准备好的dom，初始化echarts实例
          let userSummaryChart = echarts.init(
            document.getElementById('userSummaryChart')
          )
          // 绘制图表
      ...
      getUserCumulate({
        ...
        .then((response) => {
          ...
          // 基于准备好的dom，初始化echarts实例
          let userCumulateChart = echarts.init(
            document.getElementById('userCumulateChart')
          )
          // 绘制图表
      ...
      getInterfaceSummary({
        ...
        .then((response) => {
          ...
          // 基于准备好的dom，初始化echarts实例
          let interfaceSummaryChart = echarts.init(
            document.getElementById('interfaceSummaryChart')
          )
          // 绘制图表
          ...
          // 基于准备好的dom，初始化echarts实例
          let interfaceSummaryChart2 = echarts.init(
            document.getElementById('interfaceSummaryChart2')
          )
          // 绘制图表
