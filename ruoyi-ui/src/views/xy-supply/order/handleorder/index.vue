  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['supply:handleOrder:remove']"
          >删除</el-button
        >
      </el-col>
      <right-toolbar
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            icon="el-icon-view"
            @click="handleUpdate(scope.row)"
            v-hasPermi="['supply:handleOrder:query']"
            >预览</el-button
          >
          <el-button
            size="mini"
    ...
    >
      <div
        style="padding: 30px"
        :style="[
          { height: orderdasbord ? '70vh' : 'auto' },
          { overflow: orderdasbord ? 'auto' : '' },
        ]"
      >
        <el-form ref="form" :model="form" :rules="rules" label-width="120px">
          ...
          <el-form-item label="意向企业" prop="enterpriseName">
            <el-input
              :disabled="orderdasbord"
            ...
            <el-col :span="12">
              <el-form-item label="联系人" prop="enterpriseContactPerson">
                <el-input
                  ...
                  placeholder="请输入意向企业联系人"
                />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="联系方式" prop="enterpriseContactPhone">
                <el-input
                  :disabled="orderdasbord"
                  v-model="form.enterpriseContactPhone"
                  placeholder="请输入意向企业联系方式"
                />
              </el-form-item>
            </el-col>
          </el-row>
          ...
          <el-form-item label="Handle二维码地址" prop="handleQrUrl">
            <el-input
              :disabled="orderdasbord"
              v-model="form.handleQrUrl"
              placeholder="请输入Handle二维码地址"
            />
          </el-form-item>
          <el-form-item label prop="handleQrUrl" v-if="form.handleQrUrl">
            <el-image 
            :src="form.handleQrUrl" 
            style="width:128px;height:128px"
            class="tablePic" />
          </el-form-item>
          <el-form-item label="产品图片" prop="cover" v-if="!form.cover">
            <el-image
              class="tablePic"
              src=""
            />
          ...
          <el-form-item label="产品图片" prop="cover" v-if="form.cover">
            <el-image
              :src="item"
              class="tablePic"
            style="width:128px;height:128px;margin-right:24px"
              v-for="(item, index) in form.cover.split(',')"
              :key="index"
            />
          </el-form-item>
...
import {
  ...
  addHandleOrder,
  updateHandleOrder,
} from "@/api/xy-supply/order/handleOrder";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listHandleOrder(this.queryParams).then((response) => {
    ...
    handleUpdate(row) {
      ...
      getHandleOrder(id).then((response) => {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除所选产品订单？')
        .then(function () {
          return delHandleOrder(ids);
        })
        .then(() => {
          this.getList();
