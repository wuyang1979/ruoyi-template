<template>
  <div class="app-container">
    <el-form
      ...
      label-width="96px"
    >
      <el-form-item label="政策解读名称" prop="title">
        <el-input
          v-model="queryParams.title"
          placeholder="请输入政策解读名称"
          clearable
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['thinkTank:interpret:add']"
        >新增</el-button>
      </el-col>
      ...
      <!-- <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['thinkTank:interpret:remove']"
        >删除</el-button>
      </el-col>-->
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="interpretList" @selection-change="handleSelectionChange">
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['thinkTank:interpret:release']"
          >发布</el-button>
          <el-button
            ...
            v-hasPermi="['thinkTank:interpret:cancel']"
          >取消发布</el-button>
          <el-button
            size="mini"
            type="text"
            ...
            v-hasPermi="['thinkTank:interpret:edit']"
          >修改</el-button>
          <el-button
            size="mini"
            type="text"
            ...
            v-if="
              ...
              scope.row.status == '5'
            "
            v-hasPermi="['thinkTank:interpret:submit']"
          >提交审核</el-button>
          <el-button
            size="mini"
            type="text"
            ...
            v-hasPermi="['thinkTank:interpret:withdraw']"
          >撤回</el-button>
          <el-button
            size="mini"
            type="text"
            icon="el-icon-view"
            @click="handlesee(scope.row)"
            v-hasPermi="['thinkTank:interpret:query']"
          >预览</el-button>
          <el-button
            size="mini"
            type="text"
    ...
    >
      <el-form
        ...
        label-width="120px"
      >
        <el-form-item label="原政策标题" prop="policyTitle" width="120">
          <el-input v-model="form.policyTitle" placeholder="请输入原政策标题" maxlength="100"
                show-word-limit/>
        </el-form-item>

        <el-form-item label="政策链接地址" prop="policyUrl">
          <el-input v-model="form.policyUrl" placeholder="请输入政策链接地址" />
        </el-form-item>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="分类" prop="interpretTypeId" required>
              <TreeSelecter
                :props="treeSelectProps"
                v-model="form.interpretTypeId"
                :options="treeSelectOptions"
                :value="form.interpretTypeId"
              ></TreeSelecter>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    >
      <div style="height: 70vh;overflow:auto;padding: 30px">
        <el-form ref="form" :model="form" :rules="rules" label-width="80px" class="formll">
          ...
          <div class="laiyuan" v-if="form.createTime">来源：{{ form.source }}</div>
        </el-form>
      </div>
    </el-dialog>
    <el-dialog
      :close-on-click-modal="false"
      :visible.sync="opensrc"
      width="900px"
      append-to-body
      class="dialog"
    >
      <el-form ref="form" :model="form" :rules="rules" label-width="80px" class="formll">
        <div>
          <img class="morensrc" src="@/assets/images/profile.jpg" alt />
        </div>
      </el-form>
    </el-dialog>
...
import {
  ...
  withdraw,
  updateRecTop,
} from '@/api/xy-think-tank/interpret'
export default {
  name: 'Interpret',
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        policyTitle: [
          { required: true, message: '原政策标题不能为空', trigger: 'blur' },
        ],
        ...
        policyUrl: [
          { required: true, message: '政策地址不能为空', trigger: 'blur' },
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listInterpret(this.queryParams).then((response) => {
        response.rows.forEach((element) => {
          if (element.cover !== null) {
            element.cover = element.cover.split(',')
          }
        })
        this.interpretList = response.rows
    ...
    // 跳转
    handleherf(row) {
      window.open(row.policyUrl, '_blank')
    },
    // 预览按钮
    handlesee(row) {
      this.reset()
      const id = row.id || this.ids
      getInterpret(id).then((response) => {
    ...
    submit() {
      ...
      fabuInterpret(add).then((response) => {
        if (add.status == '2') {
    ...
    fenlei() {
      fenleiPolicy(this.queryParams).then((res) => {
        ...
        console.log('this.treeSelectOptions: ', this.treeSelectOptions)
