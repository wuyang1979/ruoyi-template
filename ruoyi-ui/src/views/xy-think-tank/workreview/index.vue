  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item>
        ...
        <el-button
          ...
          size="mini"
          @click="resetparam('0')"
          v-hasPermi="['thinkTank:expertWorks:batch']"
        >批量通过</el-button>
        <!-- <el-button
          :disabled="single"
          type="danger"
    ...
    <el-table v-loading="loading" :data="newsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column
        ...
        :show-overflow-tooltip="true"
      />
      <el-table-column label="出版人" align="center" prop="publishBy" width="120" />
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            type="text"
            icon="el-icon-check"
            v-hasPermi="['thinkTank:expertWorks:approval']"
...
import {
  ...
  updetaWorks,
  getexpertworks,
} from '@/api/xy-think-tank/workreview'
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listexpertWorks(this.queryParams).then((response) => {
    ...
    handlesee(row) {
      this.reset()
      const id = row.id || this.ids
      getexpertworks(id).then((response) => {
    ...
    submit() {
      ...
      updetaexpertWorks(add).then((response) => {
        this.$modal.msgSuccess('审核成功')
        this.opentwo = false
